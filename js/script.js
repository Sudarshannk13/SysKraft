$('.slideshow').imagesLoaded({ background: true })
  .done( function() {
    ...
    navigationItem.each(function(index, elem) {
      console.log(index * 360 / navigationItem.length);
      TweenMax.set(elem, {
        left: navigation.width() / 2 - navigationItem.width() / 2 - 10,
        rotation: (window.innerWidth <= 768 ? 0 : -90) + (index * 360 / navigationItem.length),
        ...
        .staggerTo(navigationItem.find('.background-holder'), 0.6, {
          cycle: {
            ...
            rotation: function(index, element) {
              //console.log(-90 - Number($(element).prev('.rotate-holder').text()) + rotation + type);
              return (window.innerWidth <= 768 ? 0 : 90) - Number($(element).prev('.rotate-holder').text()) + rotation + type;
            }
          },
          transformOrigin: "50% 50%",
          ease: Sine.easeInOut,
